...
# XAUUSD: XAUT-USDT (spot gold proxy), M5 + M15
WATCH = [("XAUT-USDT", "5m"), ("XAUT-USDT", "15m")]
...
def formed_msg(ana, sig, wait):
    ...
    lines = [f"🔵 SETUP BARU · XAUUSD-spot {sig['bar']} · {arah}",
             f"POI fresh: {f(sig['zl'])} – {f(sig['zh'])} (candle {t(sig['ts'])} WIB)",
             f"Entry limit {f(sig['entry'])} · SL {f(sig['sl'])} · TP {f(sig['tp'])}"
             f" · RR 1:{sig['rr']:.2f}",
             f"Harga {f(ana['cmp'])} — jarak ~{sig['dist']:.1f} ATR {side}. "
             "Pasang limit + alarm di chart MT4/5."]
    if wait:
        lines.append(f"⏳ TUNGGU: IDM {f(sig['idm'])} belum tersapu — entry baru sah "
                     f"setelah harga {'turun menyapu' if bull else 'naik menyapu'} level itu.")
...
def near_msg(ana, sig):
    f = ana["f"]
    return (f"🟡 MENDEKAT · XAUUSD {sig['bar']} · {sig['d'].upper()}\n"
            f"Harga {f(ana['cmp'])}, ~{sig['dist']:.1f} ATR lagi ke POI "
            f"{f(sig['zl'])} – {f(sig['zh'])}.\n"
            f"Entry {f(sig['entry'])} · SL {f(sig['sl'])} · TP {f(sig['tp'])} · "
            f"RR 1:{sig['rr']:.2f} — pastikan limit sudah terpasang.")


def entry_msg(ana, sig, idm_ok):
    f = ana["f"]
    lines = [f"🟢 ENTRY · XAUUSD {sig['bar']} · {sig['d'].upper()} — "
    ...
    if idm_ok is True:
        lines.append("IDM tersapu ✔ — kondisi entry CTC terpenuhi.")
    elif idm_ok is False:
        lines.append("⚠️ IDM belum tersapu — entry belum sah menurut metode, hati-hati.")
    return "\n".join(lines)


def ready_msg(ana, sig, near):
    f = ana["f"]
    lines = [f"🟢 IDM TERSAPU · XAUUSD {sig['bar']} · {sig['d'].upper()} — "
             f"gate entry terpenuhi",
